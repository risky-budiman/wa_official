...
<div class="h-full flex overflow-hidden bg-slate-100/70 dark:bg-slate-950 text-slate-900 dark:text-slate-100">
  ...
  <div class="flex-1 flex flex-col bg-white dark:bg-slate-950 min-w-0">
    {#if selectedConv}
      <!-- Chat Header (Clean & Non-overlapping) -->
      <div class="h-16 px-5 border-b border-slate-200 dark:border-slate-800/80 bg-white/95 dark:bg-slate-900/60 flex items-center justify-between gap-3 shrink-0">
        <!-- Left: Contact Details -->
        <div class="flex items-center gap-3 min-w-0 flex-1">
          <div class="w-9 h-9 rounded-full bg-slate-200 dark:bg-slate-700 flex items-center justify-center font-bold text-sm text-emerald-700 dark:text-emerald-400 border border-slate-300 dark:border-slate-600 shrink-0">
          ...
          <div class="min-w-0 flex-1">
            ...
            <p class="text-[11px] text-slate-500 dark:text-slate-400 truncate">
              +{selectedConv.contact.waId} • <span class="text-slate-700 dark:text-slate-300 font-medium">{selectedConv.assignedUser?.fullName || 'Belum di-assign'}</span>
